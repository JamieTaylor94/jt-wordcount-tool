...
public class ByteCounterTests
{
    ...
    public void TextFile_Returns_Correct_Result()
    {
        ...
        var options = new WordCountOptions(new[] { "-c", fileName });
        var service = new WordCountService(options, new FileTextReader(fileName));

        var result = service.GetCount();

        Assert.Equal("341836 " + fileName, result);
    }

    [Fact]
    public void Given_ValidFileWithText_Return_ByteCount()
    {
        var filePath = SharedUtilities.CreateFile("Hello world.");
        var options = new WordCountOptions(new[] { "-c", filePath });
